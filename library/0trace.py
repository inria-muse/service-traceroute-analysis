...
class AckProbe(threading.Thread):
    ...
    def run(self):
        for i in xrange(1, SEQ_DELTA+1):
            # print("Debug #1.1 (index {})".format(i))
            self.tcp.seq += i
            # print("Debug #1.2 (index {})".format(i))
            for j in xrange(MAX_HOPS):
                # print("Debug #1.3 (index {}, {})".format(i, j))
                self.ip.id = j
                self.ip.ttl = j
                # print("Debug #1.4 (index {}, {}) - Str(ip) = '{}'".format(i, j, str(self.ip)))
                buf = dnet.ip_checksum("")
                # print("Debug #1.5 (index {}, {})".format(i, j))
                self.sock.send(buf)
                # print("Debug #1.6 (index {}, {})".format(i, j))
                time.sleep(0.001)
                # print("Debug #1.7 (index {}, {})".format(i, j))
