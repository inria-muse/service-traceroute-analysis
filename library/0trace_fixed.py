    ...
    def run(self):
        for i in xrange(1, SEQ_DELTA+1):
            ...
            for j in xrange(MAX_HOPS):
                ...
                dst = socket.inet_ntoa(self.ip.dst)
                src = socket.inet_ntoa(self.ip.src)
                pkt = IP(dst=dst, src=src)
                pkt.id = j
                pkt.ttl = j

                tcp = TCP(sport=self.tcp.sport, dport=self.tcp.dport)
                tcp.flags = 0x10 #ACK
                tcp.seq = self.tcp.seq
                tcp.ack = self.tcp.ack

                # buf = dnet.ip_checksum("")
                # self.sock.send(buf)
                send(pkt/tcp, verbose=0)
                time.sleep(0.001)
